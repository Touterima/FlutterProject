...
class MapService {
  ...
  Future<Map<String, double>?> getCityCoordinates(String cityName) async {
    try {
      ...
      if (kDebugMode) print('❌ Coordonnées non trouvées pour: $cityName');
      return null;
    } catch (e) {
      if (kDebugMode) print('❌ Erreur géocodage: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> calculateRoute(
    ...
    try {
      ...
      if (fromCoords == null || toCoords == null) {
        if (kDebugMode) print('❌ Coordonnées introuvables');
        return null;
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['code'] == 'Ok' && data['routes'].isNotEmpty) {
          ...
          final durationMin = (route['duration'] ?? 0) / 60;

          // Décodage sûr des coordonnées GeoJSON
          final coords = (route['geometry']['coordinates'] as List)
              .map((coord) => [
                    (coord as List)[0].toDouble(),
                    (coord as List)[1].toDouble(),
                  ])
              .toList();

          if (kDebugMode) {
            print('✅ Itinéraire calculé: ${fromCity} → ${toCity}');
          ...
          return {
            ...
            'to_coords': toCoords,
            'geometry': coords, // ✅ corrigé pour format lisible par FlutterMap
          };
        }
      }

      if (kDebugMode) {
        print('❌ Erreur OSRM (${response.statusCode}) : ${response.body}');
      }
      return null;
    } catch (e) {
      if (kDebugMode) print('❌ Exception calcul itinéraire: $e');
      return null;
    }
  }
  ...
  String _formatDuration(double minutes) {
    ...
    return remainingMinutes < 1
        ? '${hours}h'
        : '${hours}h ${remainingMinutes.toStringAsFixed(0)}min';
  }
...
class EnhancedTripService {
  ...
  Future<Map<String, dynamic>> addTripWithRoute(TripModel trip) async {
    try {
      ...
      if (kDebugMode) print('❌ Erreur ajout trajet: $e');
  ...
  Future<Map<String, dynamic>?> getTripWithRoute(String tripId) async {
    try {
      ...
      final routeData = await _mapService.calculateRoute(trip.from, trip.to);
      return {'trip': trip, 'route': routeData};
    } catch (e) {
      if (kDebugMode) print('❌ Erreur récupération trajet: $e');
      return null;
    }
  }
}
